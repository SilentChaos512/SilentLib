...
public final class GuideBookUtils {
  ...
  public static GuiPage createBookletGuiFromPage(GuideBook book, GuiScreen previousScreen,
      IGuidePage page) {

    GuiMainPage mainPage = new GuiMainPage(book, previousScreen);

    IGuideChapter chapter = page.getChapter();
    GuiEntry entry = new GuiEntry(book, previousScreen, mainPage, chapter.getEntry(), chapter, "",
        false);
  ...
  @SideOnly(Side.CLIENT)
  public static GuiPage createPageGui(GuideBook book, GuiScreen previousScreen,
      GuiGuideBase parentPage, IGuidePage page) {
  ...
  /**
   * Sometimes the GUIs don't get the GuideBook they are supposed to be initialized with. In those cases, we can get the
