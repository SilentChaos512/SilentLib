...
public class Color {
    ...
    public static DataResult<Color> read(String str) {
        try {
            return DataResult.success(parse(str));
        } catch (Exception ex) {
            return DataResult.error(() -> "Not a valid color string: " + str + " " + ex.getMessage());
        }
    }

    public static Color read(FriendlyByteBuf buf) {
        return new Color(buf.readVarInt());
    }
    ...
    public String format() {
        return format(this.color);
    }

    /**
     * Format the color as it would appear in a JSON or config file.
     *
    ...
    /**
     * Attempt to parse the string as a color. It may be either a common CSS color name or a hex code with optional
     * leading '#' or '0x'.
     *
     ...
     * <p><strong>Implementation Note:</strong> Uses {@link UnsignedInts#parseUnsignedInt(String, int)} for parsing.
     */
    ...
    /**
     * Attempt to parse the string as a color, but returns the integer representation instead of creating a Color
     * object. It may be either a common CSS color name or a hex code with optional leading '#' or '0x'.
     *
     * @param str The string to try to parse
     * @return A Color object based on str
     * @throws NumberFormatException If the string cannot be parsed
    ...
    /**
     * Validator for JSON and configs. Accepts CSS color names or hex codes with optional leading '#' or '0x'.
     *
