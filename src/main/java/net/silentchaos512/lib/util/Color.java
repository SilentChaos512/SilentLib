...
public class Color {
    ...
    public static LazyLoadBase<Color> define(ForgeConfigSpec.Builder builder, String path, int defaultValue, String comment) {
        return define(builder, path, format(defaultValue), comment);
    }

    public static LazyLoadBase<Color> define(ForgeConfigSpec.Builder builder, String path, String defaultValue, String comment) {
        if (!comment.isEmpty()) builder.comment(comment);
        ForgeConfigSpec.ConfigValue<String> config = builder.define(path, defaultValue, o ->
                o instanceof String && validate((String) o));
        return new LazyLoadBase<>(() -> parse(config.get()));
    }

    public static LazyLoadBase<Integer> defineInt(ForgeConfigSpec.Builder builder, String path, int defaultValue, String comment) {
        return defineInt(builder, path, format(defaultValue), comment);
    }

    public static LazyLoadBase<Integer> defineInt(ForgeConfigSpec.Builder builder, String path, String defaultValue, String comment) {
        if (!comment.isEmpty()) builder.comment(comment);
        ForgeConfigSpec.ConfigValue<String> config = builder.define(path, defaultValue, o ->
                o instanceof String && validate((String) o));
        return new LazyLoadBase<>(() -> parse(config.get()).getColor());
    }
