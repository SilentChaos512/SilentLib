...
public class GuiPage extends GuiGuide {
  ...
  public GuiPage(GuideBook book, GuiScreen previousScreen, GuiGuideBase parentPage,
      IGuidePage page1, IGuidePage page2) {
    ...
    if (links != null && !links.isEmpty()) {
      this.buttonViewOnline = new TexturedButton(book.getResourceGadgets(), -782822,
          ...
          Collections.singletonList(
              TextFormatting.GOLD + book.loc.getLocalizedString("guide", "onlineButton.name")));
  ...
  public void drawScreenPre(int mouseX, int mouseY, float partialTicks) {
    ...
    for (int i = 0; i < this.pages.length; i++) {
      ...
      if (page != null) {
        ...
        page.drawScreenPre(this, this.guiLeft + 6 + i * 142, this.guiTop + 7, mouseX, mouseY,
            partialTicks);
      }
    }
    for (ItemDisplay display : this.itemDisplays) {
  ...
  public void addOrModifyItemRenderer(ItemStack renderedStack, int x, int y, float scale,
    ...
    this.itemDisplays
        .add(new ItemDisplay(book, this, x, y, scale, renderedStack, shouldTryTransfer));
  }
